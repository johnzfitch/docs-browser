...
Close the current tab and all splits therein _or_ close all tabs and splits thein of tabs _other_ than the current tab, depending on the mode.
...
As quick terminals are often useful when other windows are currently focused, they are best used with _global_ keybinds. For example, one can define the following key bind to toggle the quick terminal from anywhere within the system by pressing `` Cmd+` ``:

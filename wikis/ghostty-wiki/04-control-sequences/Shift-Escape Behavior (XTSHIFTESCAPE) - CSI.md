...
When a terminal program requests [mouse reporting](https://ghostty.org/docs/features/theme#TODO), some mouse reporting modes also report the modifier keys that are pressed (control, shift, etc.). This would disable the ability for a terminal user to natively select text if they typically select text using left-click and drag, since the left-click event is captured by the running program.

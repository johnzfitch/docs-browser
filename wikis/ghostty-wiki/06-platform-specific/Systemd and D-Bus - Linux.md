If properly installed<sup><a href="https://ghostty.org/docs/config/keybind#user-content-fn-1" id="user-content-fnref-1" data-footnote-ref="true" aria-describedby="footnote-label">1</a></sup>, Ghostty comes with several systemd units. Running Ghostty via these systemd units is the recommended way to run Ghostty. The two most important benefits provided by Ghostty's systemd integrations are: instantaneous launching<sup><a href="https://ghostty.org/docs/config/keybind#user-content-fn-2" id="user-content-fnref-2" data-footnote-ref="true" aria-describedby="footnote-label">2</a></sup> and centralized logging.

If you are on a system with `systemd` and you installed Ghostty via a system package or [as documented](https://ghostty.org/docs/install/build), then Ghostty should _just work_. You don't have to read this documentation unless you want to understand how this all works or if you're troubleshooting.
...
> This will gracefully stop Ghostty, but Ghostty will close any windows and kill their running processes _without asking for confirmation_. This may cause data or state loss.

If properly installed <sup><a href="https://ghostty.org/docs/config/keybind#user-content-fn-1" id="user-content-fnref-1-2" data-footnote-ref="true" aria-describedby="footnote-label">1</a></sup>, Ghostty will come with several configuration files to launch Ghostty.
...
1.  "Properly installed" means via a package that followed our packaging guidelines or from source in `$HOME/.local`. [↩](https://ghostty.org/docs/config/keybind#user-content-fnref-1) [↩<sup>2</sup>](https://ghostty.org/docs/config/keybind#user-content-fnref-1-2)
    
2.  "Instantaneous" is hyperbolic, of course. Practically speaking, launching a new Ghostty window goes from somewhere around 300ms on a modern system to around 20ms. These numbers will vary based on hardware, but the relative increase should be noticeable on all systems. [↩](https://ghostty.org/docs/config/keybind#user-content-fnref-2)
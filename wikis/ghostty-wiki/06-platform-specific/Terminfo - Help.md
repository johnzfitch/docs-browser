...
> **Why the `xterm` prefix?** Unfortunately, many terminal applications string search for "xterm" in the `TERM` value to _assume capabilities_. During the private beta, we experimented with a pure `ghostty` value, but found that too many applications broke and fixing the entire ecosystem was unrealistic. We settled on `xterm-ghostty` to maintain compatibility.
